...
void InvestigatorOrchestartor::createInitialAvsProcess(bool needLaterStart) {
    ...
    connect(m_initialAvsProcess,  QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
        ...
        if(exitStatus == QProcess::NormalExit) {
            ...
            if(needLaterStart) {
                log("Автоматическое возобновление работы программы после перезапуска.", Logger::FILE + Logger::UI + Logger::SYSLOG);
                startWork();
            }
            ...
            */
            ...
            log(QString("Процесс инициализирующей проверки был аварийно завершен."), Logger::UI + Logger::SYSLOG);
        }
    }
    );
}
...
void InvestigatorOrchestartor::createWorkers() {
    ...
    for(int i = 0; i < MaxThreadNb; i++) {
        ...
        connect(this,           &InvestigatorOrchestartor::tryWorkerCheckFiles,     m_workers[i],   &InvestigatorWorker::tryCheckFiles);
        connect(m_workers[i],   &InvestigatorWorker::finish,                        this,           &InvestigatorOrchestartor::onWorkerFinished);
        connect(m_workers[i],   &InvestigatorWorker::log,                           this,           &InvestigatorOrchestartor::log);
...
void InvestigatorOrchestartor::createWorkTimer() {
    ...
    connect(m_workTimer, &QTimer::timeout, [=]() {
        ...
        if(m_isInWork) {
            // 0 - ss, 1 - mm, 2 - hh, 3 - dd
            m_workTimeV[0]++;
            if(m_workTimeV[0] / 60) { m_workTimeV[1]++; m_workTimeV[0] %= 60;}
            if(m_workTimeV[1] / 60) { m_workTimeV[2]++; m_workTimeV[1] %= 60;}
            if(m_workTimeV[2] / 24) { m_workTimeV[3]++; m_workTimeV[2] %= 24;}
        }
    });
...
bool InvestigatorOrchestartor::reconfigureWorkers() {
    ...
    if(!QDir(m_infectedDir).exists() || m_infectedDir.isEmpty()) {
        log("Не найдена директория для зараженных файлов.", Logger::FILE + Logger::UI);
        errorExist = true;
    }

    if(!QDir(m_cleanDir).exists() || m_cleanDir.isEmpty()) {
        log("Не найдена директория для чистых файлов.", Logger::FILE + Logger::UI);
        errorExist = true;
    }
    ...
    if(m_tempCurrentWorkersNb < 1 || m_tempCurrentWorkersNb > MaxThreadNb) {
        log("Указано некорректное количество воркеров.", Logger::FILE + Logger::UI);
        errorExist = true;
    }

    if(m_thresholdFilesNb < 1) {
        log("Указано некорректное значение порога по количеству файлов.", Logger::FILE + Logger::UI);
        errorExist = true;
    }

    if(m_thresholdFilesSize < 1) {
        log("Указано некорректное значение порога по объему файлов.", Logger::FILE + Logger::UI);
        errorExist = true;
    }
    ...
    if(!errorExist) {
        ...
        for(int i = 0; i < MaxThreadNb; i++) {
            ...
            m_workers[i]->configure(i, QStringList{m_sourceDir, m_processDir, m_infectedDir, m_cleanDir, m_processDir + "\\reports\\"}, m_avsExecFileName);
    ...
    log(QString("В процессе конфигурирования воркеров возникли ошибки."), Logger::FILE);
    return false;
}
...
void InvestigatorOrchestartor::fileLog(QString message) {
    ...
    if(m_logFile.fileName() == outFileName) {
        ...
        if(m_logFile.exists()) {
            ...
            if(m_logFile.isOpen() && QFileInfo(m_logFile).fileTime(QFileDevice::FileModificationTime).msecsTo(QDateTime::currentDateTime()) > 5000) {
                m_logFile.close();
            }
        }
    } else {
...
bool InvestigatorOrchestartor::updateTotalFileList() {
    if(!m_totalFileList.isEmpty()) {
        ...
        log(QString("Список файлов на проверку обновлен. В очереди %1 файлов.").arg(m_totalFileList.size()), Logger::FILE);
    }
        ...
        for(int i = 0; i < m_currentWorkersNb; i++) {
            if(!m_workers[i]->isInProcess()) { // если воркер не занят
                if(updateTotalFileList()) { // если есть файлы для проверки
                    ...
                    log(QString("Воркеру %1 назначено %2 файлов на проверку.").arg(i).arg(filesToCheck.size()), Logger::FILE);
...
void InvestigatorOrchestartor::onWorkerFinished(int workerId) {
    if(workerId >= 0 && workerId < MaxThreadNb) {
        ...
        if(m_avVersion != workerStatistic.avsVersion && // если версия другая
            ...
            log(QString("Найдена новая версия баз АВС: %1.").arg(avsVersion), Logger::FILE + Logger::UI);
...
QFileInfoList InvestigatorOrchestartor::getFilesToCheckList() {
    ...
    while(m_totalFileList.size() && // пока есть файлы в m_totalFileList
          resultListSize < (m_thresholdFilesSize * m_thresholdFilesSizeUnit) && // и пока не превышен порог по объему
          quint64(resultList.size()) < m_thresholdFilesNb ) { // и пока не превышен порог по кол-ву файлов
...
void InvestigatorOrchestartor::startWork() {
    ...
    if(reconfigureWorkers()) {
        ...
        log(QString("Не удалось запустить потоковую проверку."), Logger::FILE + Logger::UI);
        m_processTimer->stop();
        m_isInWork = false;
    }
...
void InvestigatorOrchestartor::stopWork() {
    m_processTimer->stop();
    if(m_isInWork) {
        log(QString("Потоковая проверка остановлена."), Logger::FILE + Logger::UI);
    }
    m_isInWork = false;
    stopWorkers();
    dumpWorkTimer();
...
void InvestigatorOrchestartor::getInitialAvsScan(bool needLaterStart) {
    ...
    if(QFile(m_avsExecFileName).exists() && !m_avsExecFileName.isEmpty()) {
        ...
        log(QString("Начат процесс инициализирующей проверки АВС."), Logger::FILE);

        // пока не будет налажена свзяь с АВС
        while(!m_successInitialScan) {
            log(QString("Попытка запуска инициализирующей проверки АВС..."), Logger::FILE);
...
void InvestigatorOrchestartor::sendSyslogMessage(QString msg) {
    msg.prepend(QString("<%1>%2 %3 ").arg(59).arg(QNetworkInterface::allAddresses().first().toString()).arg(QDateTime::currentDateTime().toString("yyyy.MM.dd hh:mm:ss")));
